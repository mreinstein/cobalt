...
export function resize (renderer, viewportWidth, viewportHeight) {
    const { device, bloom } = renderer
    const { pipeline, params_buf } = renderer.postProcessing

    renderer.postProcessing.bindGroup = device.createBindGroup({
        layout: pipeline.getBindGroupLayout(0),
        entries: [
          {
            binding: 0,
            resource: bloom.hdr_sampler,
          },
          // color
          {
            binding: 1,
            resource: bloom.hdr_texture.view,
          },
          // emissive
          {
            binding: 2,
            resource: bloom.bind_groups_textures[2].mip_view[0],
          },
          {
            binding: 3,
            resource: {
...
export function draw (renderer, commandEncoder, view) {
